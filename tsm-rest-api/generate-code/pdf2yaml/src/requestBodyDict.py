...
def listOfPropertyEntries(title: str, reduced_dict: dict) -> list:
    ...
    for i in reduced_dict:
        ...
        if (
            i == 'cspELFsVersions'
        ) and no_break:  ## list of pairs (aid-string, version-string)
            for j in range(4):  # range(secrets.choice(range(3,7))):
                example_value += (
                    '[ "'
                    + ExampleValues.get('aid')
                    + '", "'
                    + ExampleValues.get('version')
                    + '" ], '
                )
            property_entries.append(i + ': [ ' + example_value[:-2] + ' ]')
            continue
        if (i == 'cspVendor') and no_break:
            ...
            for (
                ...
                csp_vendor_list
            ):  # random.sample(csp_vendor_list,secrets.choice(range(1,3))):
                example_value += j + ', '
            property_entries.append(i + ': [ ' + example_value[:-2] + ' ]')
            continue
        if (i == 'name') and no_break:
            if title == 'Service':
                property_entries.append(
                    i + ': "' + ExampleValues.get('service_name') + '"'
                )
            elif title in ('Flavor', 'ApplicationConfig'):
                property_entries.append(
                    i + ': "' + ExampleValues.get('flavor_name') + '"'
                )
            else:
        ...
        if (i == 'applicationInstantiationConfigs') and no_break:
            for j in range(3):  # range(secrets.choice(range(3,6))):
                example_value += (
                    '{ priority: '
                    + ExampleValues.get('int', 0, 256)
                    + ', executableModuleId: "'
                    + ExampleValues.get('uuid')
                    + '", applicationConfigId: "'
                    + ExampleValues.get('uuid')
                    + '" }, '
                )
