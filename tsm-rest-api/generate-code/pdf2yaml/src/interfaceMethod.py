    ...
    def __init__(self, interface_method_text: list):
        ...
        # Handle kapitel
            ...
            if '4.1.6.5.4' in interface_method_text[0]:
                pass
            # get specific lines
            line_rest_url = returnTextLineFromList(self.interface_method_text,'REST-URL\n')
            line_request_method = returnTextLineFromList(self.interface_method_text,'Request Method\n',line_rest_url)
            line_request_headers = returnTextLineFromList(self.interface_method_text,'Request Headers\n',line_request_method)
            ...
            if 'Request Body\n' in self.interface_method_text:
                line_request_body = returnTextLineFromList(self.interface_method_text,'Request Body\n',line_request_headers)
                ...
                if 'Request ELF-File\n' in self.interface_method_text:
                    ...
                    line_request_file = returnTextLineFromList(self.interface_method_text,'Request ELF-File\n',line_request_headers)
                elif 'Request Script-File\n' in self.interface_method_text:
                    #line_request_filename = returnTextLineFromList(self.interface_method_text,'Request Script-Filename\n',line_request_headers)
                    line_request_file = returnTextLineFromList(self.interface_method_text,'Request Script-File\n',line_request_headers)
                elif 'Request Cert-File\n' in self.interface_method_text:
                    #line_request_filename = returnTextLineFromList(self.interface_method_text,'Request Cert-Filename\n',line_request_headers)
                    line_request_file = returnTextLineFromList(self.interface_method_text,'Request Cert-File\n',line_request_headers)
                line_request_headers_end = line_request_file
            line_response_success = returnTextLineFromList(self.interface_method_text,'Response on success:\n',line_request_body)
            line_response_headers_succ = returnTextLineFromList(self.interface_method_text,'Response Headers\n',line_response_success)
            line_response_body_succ = returnTextLineFromList(self.interface_method_text,'Response Body\n',line_response_success)
            line_response_failures = returnTextLineFromList(self.interface_method_text,'Responses on failures:\n',line_response_success)
            line_response_headers_fail = returnTextLineFromList(self.interface_method_text,'Response Headers\n',line_response_failures)
            line_response_body_fail = returnTextLineFromList(self.interface_method_text,'Response Body\n',line_response_failures)

            # extract information at specific lines

            # rest-url
            rest_url_list = self.interface_method_text[line_rest_url+1:line_request_method]
            self.rest_url = ''
            for url_line in rest_url_list:
                self.rest_url += url_line.replace('<<tsmBaseURL>>','').replace(' ','').strip()
            if self.rest_url[-1] == '/':
                self.rest_url=self.rest_url[:-1]
            # tag
            self.tag = '/'+self.rest_url.split('/')[1]
            # request method
            self.request_method = self.interface_method_text[line_request_method+1].lower().strip()
            # description
            self.description = ''
            ...
            for i in range(line_request_headers+1,line_request_headers_end,2):
                self.request_headers[self.interface_method_text[i].strip()] = self.interface_method_text[i+1].strip()
            # request filename
            ...
            if line_request_file:
                ...
                for i in range(line_request_file+1,line_response_success,2):
                    self.request_file[self.interface_method_text[i].strip()] = self.interface_method_text[i+1].strip()
            else:
            ...
            if line_request_body:
                if self.interface_method_text[line_request_body+1].strip() == '-':
                    ...
                    self.request_body = self.interface_method_text[line_request_body+1:line_response_success]
                    ...
                    if '<' in self.request_body[0] and '>' in self.request_body[0]:
                        for i, req in enumerate(self.request_body):
                            ...
                            if ':' not in req and '<' not in req:
                                self.request_body[i-1] = self.request_body[i-1].strip() + ' ' + req
                                self.request_body[i] = ''
                        # pull whole description together
                        if len(self.request_body) > 2:
                            for i in range(2,len(self.request_body)):
                                self.request_body[1] = self.request_body[1].strip() + ', ' + self.request_body[i]
                                self.request_body[i] = ''
            ...
            for i in re_parameters:
                self.parameters.append('- $ref: "#/components/parameters/'+i[1:-1]+'"')
            # response headers (success)
            self.response_headers_success = {}
            for i in range(line_response_headers_succ+1,line_response_body_succ,2):
                self.response_headers_success[self.interface_method_text[i].strip()] = self.interface_method_text[i+1].strip()
            # response body (success)
            self.response_body_success = self.interface_method_text[line_response_body_succ+1].strip()
            if 'EntityList<' in self.response_body_success:
                self.response_body_success = self.response_body_success.replace('EntityList<','').replace('>','[]')
            ...
            for i in range(line_response_headers_fail+1,line_response_body_fail,2):
                self.response_headers_failure[self.interface_method_text[i].strip()] = self.interface_method_text[i+1].strip()
            self.response_headers_failure['Status Code'] = [x.strip() for x in self.response_headers_failure['Status Code'].split(',')]
            # response body (failure) == general error
            self.response_body_failure = {}
            self.response_body_failure['GeneralError'] = []
            temp_list = [x.strip() for x in self.interface_method_text[line_response_body_fail+2].split(',')]
            ...
            self.response_body_failure['GeneralError'] = sorted(self.response_body_failure['GeneralError'])
